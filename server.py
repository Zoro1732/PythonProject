...
def handle_upload_avatar(username, b64data, addr, sock):
    ...
    try:
        ...
        send(addr, sock, "UPLOAD_FAIL|write_error")
        print("Failed to write avatar:", e)
        return
    users[username]["avatar"] = path
    send(addr, sock, "UPLOAD_OK")
    print(f"Avatar saved for {username} -> {path}")

def handle_get_avatar(target, addr, sock):
    path = os.path.join(AVATAR_FOLDER, f"{target}.jpg")
    if not os.path.exists(path):
        send(addr, sock, "AVATAR_NOT_FOUND")
        return
    with open(path, "rb") as f:
        b64 = base64.b64encode(f.read()).decode()
    send(addr, sock, f"AVATAR|{target}|{b64}")
        ...
        try:
            ...
            if cmd == "LOGIN":
                ...
                if user is None:
                    ...
                    print(f"Failed login attempt for {username} (unknown user)")
                    continue

                if user["lives"] <= 0:
                    # player has no lives -> game over
                    send(addr, sock, "LOGIN_FAIL|game_over")
                    print(f"User {username} attempted login but is game-over (lives=0).")
                    continue
                ...
                if user["password"] == password:
                    ...
                    send(addr, sock, "LOGIN_FAIL|bad_credentials")
                    print(f"Failed login attempt for {username} (bad password)")
